...
class FinanceApp {
    ...
    async handleFiles(files) {
        ...
        for (const file of files) {
            try {
                ...
                if (rulesResult.applied > 0) {
                    importMsg += `\n✨ ${rulesResult.applied} categorizadas automáticamente`;
                }

                this.showNotification(importMsg, uniqueTransactions.length > 0 ? 'success' : 'warning');
        ...
        if (totalParsed > 0 || totalDuplicates > 0) {
            let summaryMsg = `📊 Resumen de importación:\n`;
            summaryMsg += `✅ ${totalParsed} transacciones nuevas agregadas\n`;
            
            if (totalDuplicates > 0) {
                summaryMsg += `⚠️ ${totalDuplicates} duplicados detectados y omitidos\n`;
            }
            
            summaryMsg += `📁 ${filesProcessed} archivo(s) procesado(s)`;

            this.showNotification(summaryMsg, 'success');
            this.saveToStorage();
            this.updateUI();
    ...
    clearAllData() {
        ...
        const confirmText = `Vas a eliminar ${numTransactions} transacciones.\n\nEscribe "BORRAR" para confirmar:`;
    ...
    updateDashboardView() {
        this.populateDashboardMonthFilter();
    ...
    populateDashboardMonthFilter() {
        ...
        const months = this.getUniqueMonths(this.transactions);
        
        let customOptions = months.map(monthKey => {
    ...
    filterDashboardByMonth(filter) {
        ...
        switch(filter) {
            case 'all':
                ...
                break;
            case 'current':
                filteredTransactions = this.getCurrentMonthTransactions();
                break;
            case 'last':
                filteredTransactions = this.getLastMonthTransactions();
                break;
            default:
                filteredTransactions = this.getMonthTransactions(filter);
        }

        this.updateDashboardWithFilteredData(filteredTransactions);
    }
    ...
    updateDashboardWithFilteredData(transactions) {
        ...
        this.charts.updateMonthlyChart(transactions);
        this.charts.updateCategoryChart(transactions);
        this.charts.updateBalanceChart(transactions);
        this.charts.updateTopCategoriesChart(transactions);
    }
    ...
    calculateStatistics(transactions = null) {
        ...
        try {
            if (this.profileManager.isLoggedIn()) {
                const data = {
                    ...
                    rules: this.rulesManager.rules
                };
                this.profileManager.saveProfileData(data);
            } else {
    ...
    loadFromStorage() {
        try {
            if (this.profileManager.isLoggedIn()) {
                const data = this.profileManager.loadProfileData();
                
                if (data) {
                    this.transactions = data.transactions || [];
                    
                    if (data.rules) {
                        this.rulesManager.rules = data.rules;
                    }
                    
                    console.log(`✅ Cargadas ${this.transactions.length} transacciones del perfil`);
                } else {
                    this.transactions = [];
                }
            } else {
